...
class ReadingService {
     ...
     * 删除书籍（软删除）
     */
    async deleteBook(bookId, schoolId) {
     ...
     * 获取学生阅读统计
     */
    async getStudentReadingStats(studentId, schoolId) {
        // 获取所有阅读记录
     ...
     * 获取学生今日阅读记录（用于家长端公告）
     */
    async getTodayReadingLogs(studentId, schoolId) {
        // 获取今日时间范围（北京时间）
        const now = new Date();
     ...
     * 获取学生最近选择的书籍（用于默认显示）
     */
    async getLastSelectedBook(studentId, schoolId) {
