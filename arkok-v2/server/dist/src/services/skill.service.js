...
class SkillService {
    ...
    async getStudentSkills(studentId) {
        ...
        return allSkills.map(skill => {
            const progress = skillMap.get(skill.id);
    ...
    async recordPractice(params) {
        ...
        if (!skill)
            throw new Error(`技能不存在: ${skillCode}`);
        // 记录修炼
        ...
        if (newLevel > oldLevel && newLevel >= 1) {
            ...
            if (this.io) {
                this.io.emit('skill_levelup', {
                    ...
                    className: student?.className || '', // Prisma type might need include className or explicit select
     ...
     * 增加五维属性经验
     */
    async addAttributeExp(studentId, attribute, exp) {
        // 确保 stats 记录存在
     ...
     * 更新连胜记录
     */
    async updateStreak(studentId, increment = true) {
    ...
    // ========== 辅助方法 ==========
    calculateLevel(levelData, currentExp) {
        let level = 0;
    ...
    getNextLevelExp(levelData, currentLevel) {
