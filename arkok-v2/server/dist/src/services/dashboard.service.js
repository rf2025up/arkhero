...
class DashboardService {
     ...
     * 获取大屏专用数据
     */
    async getBigscreenData(schoolId) {
        ...
        const pkResults = pksData.map(pk => {
            const isAWinner = pk.winnerId === pk.studentA;
            const metadata = pk.metadata || {};
            return {
                id: pk.id,
                winner: {
                    id: isAWinner ? pk.studentA : pk.studentB,
                    name: isAWinner ? pk.playerA?.name : pk.playerB?.name,
                    avatarUrl: isAWinner ? pk.playerA?.avatarUrl : pk.playerB?.avatarUrl,
                    score: metadata.scoreA || 0
                },
                loser: {
                    id: isAWinner ? pk.studentB : pk.studentA,
                    name: isAWinner ? pk.playerB?.name : pk.playerA?.name,
                    avatarUrl: isAWinner ? pk.playerB?.avatarUrl : pk.playerA?.avatarUrl,
        ...
        const challengeResults = challengesData.map(c => ({
            id: c.id,
            studentName: c.students?.name || '未知',
            title: c.challenges?.title || '未知挑战',
            success: c.result === 'COMPLETED',
            expAwarded: c.challenges?.rewardExp || 0,
        ...
        const taskActivities = filteredTasksData.map(t => {
            const categoryMap = {
            ...
            const getSmartLabel = (taskTitle, taskCategory, content) => {
            ...
            if (contentObj.isStreak && Array.isArray(contentObj.items) && contentObj.items.length > 0) {
                // 取最新的连胜项目
                const latestStreak = [...contentObj.items].sort((a, b) => new Date(b.updatedAt || 0).getTime() - new Date(a.updatedAt || 0).getTime())[0];
                if (latestStreak) {
                    label = `【连胜荣耀】`;
            ...
            return {
                id: t.id,
                type: (categoryMap[t.task_category] || 'task'),
                studentName: t.students?.name || '未知',
                content: label + title,
        ...
        const habitActivities = habitLogsData.map((h) => ({
            id: h.id,
            type: 'habit',
        ...
        // 将阅读日志转换为 activities
        const readingActivities = readingLogs.map((r) => ({
            id: r.id,
            type: 'progress',
        ...
        // 合并并按时间排序，限制最多20条
        const activities = [...taskActivities, ...habitActivities, ...readingActivities]
        ...
        const recentBadgesList = badgesData.map(b => ({
            id: b.id,
            badgeName: b.badges?.name || '勋章',
            badgeIcon: b.badges?.icon || '🏅',
            badgeDescription: b.badges?.description || '在相应领域表现优异，获得此项荣誉。继续加油！',
            studentName: b.students?.name || '未知',
        ...
        // 格式化技能升级数据
        const recentSkillUps = skillUpsData.map((s) => {
            const levelData = s.skill?.levelData || [];
            const levelInfo = levelData.find((l) => l.lvl === s.level);
            return {
    ...
    async getDashboardData(schoolId) {
        ...
        // 格式化 PK 数据
        const ongoingPKsFormatted = ongoingPKs.map((pk) => ({
            id: pk.id,
            topic: pk.topic,
            status: pk.status,
        ...
        // 格式化挑战数据
        const recentChallengesFormatted = activeParticipants.map(p => ({
            id: p.id,
