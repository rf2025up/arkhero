...
class StudentService {
    ...
    async getStudentProfile(studentId, schoolId, userRole, userId) {
        try {
            ...
            const [student, task_records, pkMatchesAsPlayerA, pkMatchesAsPlayerB, allPkMatches, taskStats, allHabits, studentHabitLogs, latestLessonPlan, latestOverride, student_badges, radarStats, streakStats, unlockedSkills, readingStats] = await Promise.all([
                // 1. 学生基础信息
            ...
            const multiplier = school?.settings?.expMultiplier || 1.0;
            ...
            const timelineData = this.buildTimelineData(task_records, allPkRecordsWithDetails, readingStats.rawLogs || []);
            const profile = {
                // 学生基础信息
                student: {
                    ...student,
     ...
     * 构建时间轴数据
     */
    buildTimelineData(task_records, pkRecords, readingLogs = []) {
     ...
     * 🆕 计算详细连胜数据 (按科目和任务类型)
     */
    async calculateDetailedStreaks(studentId) {
        // 获取最近100条已完成的任务记录
        ...
        for (const rule of rules) {
            ...
            if (count > 0) {
                if (!streaks[rule.subject])
                    streaks[rule.subject] = [];
    ...
    getExpRequiredForLevel(level) {
        if (level <= 5)
            return 300; // 1-5级：300 exp (原30)
        if (level <= 10)
            return 500; // 6-10级：500 exp
        if (level <= 15)
            return 800; // 11-15级：800 exp
        if (level <= 20)
            return 1200; // 16-20级：1200 exp
        if (level <= 25)
            return 1600; // 21-25级：1600 exp
        if (level <= 30)
            return 2000; // 26-30级：2000 exp
        if (level <= 40)
            return 2800; // 31-40级：2800 exp
        if (level <= 50)
            return 4000; // 41-50级：4000 exp
     ...
     * 🆕 计算阅读统计数据
     */
    async calculateReadingStats(studentId, schoolId) {
    ...
    async getExpMultiplier(schoolId) {
        ...
        return school?.settings?.expMultiplier || 1.0;
    }
}
