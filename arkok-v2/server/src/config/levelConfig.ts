 ...
 */
export const EXP_MULTIPLIER = 1.0;
 ...
 */
export function getLevelInfo(totalExp: number, multiplier: number = EXP_MULTIPLIER) {
    // 应用全局倍率：经验值 / 倍率 = 调整后经验
 ...
 */
export const STREAK_MILESTONE_EXP: Record<number, number> = {
    3: 10,    // 3次连胜 +10 exp
    7: 30,    // 7次连胜 +30 exp
    14: 60,   // 14次连胜 +60 exp
 ...
 * 获取连胜里程碑奖励经验值
 */
export function getStreakMilestoneExp(streakCount: number): number | null {
    return STREAK_MILESTONE_EXP[streakCount] || null;
}
