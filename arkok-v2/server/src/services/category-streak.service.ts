...
export class CategoryStreakService {
     ...
     */
    async updateStreakByTask(studentId: string, taskTitle: string, schoolId: string) {
        // 1. 解析任务标题 -> 连胜分类
     ...
     */
    async updateStreak(studentId: string, category: string, isPerfect: boolean) {
     ...
     * 获取学生某类别的连胜数据
     */
    async getStreak(studentId: string, category: string) {
     ...
     * 获取学生所有类别的连胜数据 (带中文标签)
     */
    async getAllStreaks(studentId: string) {
        // 1. 获取学生所在学校
        ...
        return result.sort((a, b) => {
            const orderA = subjectOrder[a.subject as keyof typeof subjectOrder] || 99;
            const orderB = subjectOrder[b.subject as keyof typeof subjectOrder] || 99;
     ...
     * @param limit 返回数量
     */
    async getSubjectLeaderboard(schoolId: string, subject: string, limit = 10) {
    ...
    // 🆕 添加自定义连胜分类
    async addCategory(schoolId: string, subject: string, name: string, code: string) {
